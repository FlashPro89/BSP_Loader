...
gSizeSortTexture sstex[MAX_LIGHTMAPS_NUM];
...
void createLightmapsAtlas( unsigned int width, unsigned int height, unsigned char border )
{
	...
	for (unsigned int i = 0; i < numLightedFaces; i++)
	{
		...
		for( int y = 0; y < sstex[i].height; y++ )
		{
			for( int x = 0; x < sstex[i].width; x++ )
			{
...
void loadLighmaps()
{
	...
	for (int i = 0; i < num_faces; i++)
	{
		...
		if (s->styles[0] == 0)
		{
			...
			//if (i == 136)
			//	D3DXSaveTextureToFileA("lmap_136.bmp", D3DXIFF_BMP, pTex, 0);

			//if (i == 137)
			//	D3DXSaveTextureToFileA("lmap_137.bmp", D3DXIFF_BMP, pTex, 0);

			//if (i == 138)
			//	D3DXSaveTextureToFileA("lmap_138.bmp", D3DXIFF_BMP, pTex, 0);
...
void loadScene( const char* mapname )
{
	...
	*/
	...
	FILE* f = 0;
	//fopen_s( &f, "../data/maps/t0a0b.bsp", "rb" );
