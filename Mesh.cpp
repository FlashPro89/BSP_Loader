...
bool gResourceSkinAnimation::load()
{
	...
	while (fgets(buffer, BUFSZ, f))
	{
		...
		q = D3DXQUATERNION(-q.x, -q.z, -q.y, q.w); 	// мен€ем ось Z на ось Y!!!
...
void gResourceSkinAnimation::unload() //данные, загруженые preload() в этой функции не измен€ютс€
{
	if (m_absMats)
		delete[] m_absMats;
	m_absMats = 0;
...
gSkinBone* gResourceSkinAnimation::getFrameInTimePos( float time, gSkinBone* frame )
{
	...
	for (int i = 0; i < m_bonesNum; i++) // интерпол€тор
	{
		...
		D3DXQuaternionSlerp(&q, &q1, &q2, i2); // можно упростить линейной интерпол€цией
		...
		if (!strncmp(buffer, "triangles", 9))
		{
			...
			while (fgets(buffer, BUFSZ, f))
			{
				...
				if (cit == m_trisCacher.end())
				{
					...
					tg.trisNum = 1; // прибавл€ем сдесь 1!
					tg.__used_tris = 0;
...
bool gResourceSkinnedMesh::load()
{
	...
	while (fgets(buffer, BUFSZ, f))
	{
		...
		// смещение дл€ записи в вершинный буффер дл€ данного треугольника
		int vdi = cit->second.__before * 3 + cit->second.__used_tris * 3;

		if (cit == m_trisCacher.end())
		{
			throw("“ут надо подумать!");
		}
		else
		{	//находим в вершинном буффере данную группу треугольников и добавл€ем к ней 
			unsigned char indexex[3];

			for (int i = 2; i >=0; i--) // мен€ем CW на CCW
			{
				if (!fgets(buffer, BUFSZ, f))
					throw("ќшибка при загрузке данных из файла!");
				...
				sscanf_s(buffer, "%u %f %f %f %f %f %f %f %f",  //FIX IT!!!
					...
					&p_vData[vdi + i].y,
					&p_vData[vdi + i].nx,
					&p_vData[vdi + i].nz,   // мен€ем оси z и y
					&p_vData[vdi + i].ny,
					&p_vData[vdi + i].tu,
					&p_vData[vdi + i].tv);
			...
			//если ожидаетс€ переполнение индексов в subset'e то создаем новый subset
			...
			//провер€ем индексы в subset'e на повтор€емость
			for (unsigned int i = 0; i < 3; i++)
			{
				auto riit = rsit->remapedIndexes.find(indexex[i] );

				//если такого индекса нет в наборе, то добавл€ем его
	...
	while ( (fgets(buffer, BUFSZ, f) ) && (unodes < m_bonesNum) )
	{
		...
		sscanf_s( buffer, "%d %f %f %f %f %f %f", &itmp,
			&pos.x, &pos.z, &pos.y, 						// мен€ем ось Z на ось Y!!!
			&rot.x, &rot.y, &rot.z);
	...
	//приготовим инверсные матрицы начального положени€
...
void gResourceSkinnedMesh::unload() //данные, загруженые preload() в этой функции не измен€ютс€
{
	m_AABB.reset();
