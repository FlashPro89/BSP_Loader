...
void _quatShortestArc(D3DXQUATERNION& out, const D3DXVECTOR3& v0, const D3DXVECTOR3& v1)
{
	...
	out.w += 1.f; // reducing angle to halfangle

	if (out.w <= 0.0000001f) // angle close to PI
	{
		if ((v0.z * v0.z) > (v0.x * v0.x))
			out = D3DXQUATERNION(0, v0.z, -v0.y, out.w); //from*vector3(1,0,0) 
		else
			out = D3DXQUATERNION(v0.y, -v0.x, 0, out.w); //from*vector3(0,0,1) 
	}
	D3DXQuaternionNormalize(&out, &out);
}
